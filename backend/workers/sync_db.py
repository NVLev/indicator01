...
def update_study_status_sync(study_id: int, status: str, error_message: str = None):
    ...
    with SessionLocal() as session:
        try:
            ...
            session.execute(
                text(
                    "UPDATE studies SET processing_status = :status, error_message = :error_message WHERE id = :id"
                ),
        ...
        try:
            ...
            heatmap_metadata_data = {
                ...
                "heatmap_data_info": {
                    ...
                    "error_map_shape": results.get("heatmap_data", {}).get(
                        "error_map_shape", []
                    ),
                    "max_error_slice_index": results.get("heatmap_data", {}).get(
                        "max_error_slice_index", 0
                    ),
                    "has_visualization": bool(
                        results.get("heatmap_data", {}).get("visualization_png")
                    ),
                },
                "heatmap_statistics": results.get("heatmap_data", {}).get(
                    "heatmap_statistics", {}
                ),
            }
            update_data = {
                ...
                "probability_of_pathology": results.get(
                    "probability_of_pathology", 0.0
                ),
                "pathology": results.get("pathology", 0),
                "most_dangerous_pathology_type": results.get(
                    "most_dangerous_pathology_type", ""
                ),
                "pathology_localization_coords": (
                    json.dumps(results.get("pathology_localization_coords"))
                    if results.get("pathology_localization_coords")
                    else None
                ),
                ...
                "needs_verification": results.get(
                    "needs_review", False
                ),  # ✅ Правильное имя поля!
                "verification_results": (
                    json.dumps(results.get("verification_results"))
            ...
            session.commit()
            logger.info(
                f"Успешно обновлены результаты исследования {study_id}, статус: {russian_status}"
            )
        except Exception as e:
            session.rollback()
